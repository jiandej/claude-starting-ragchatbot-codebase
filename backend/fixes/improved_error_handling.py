    ...
    def query(
        ...
        try:
            ...
            if not self.config.ANTHROPIC_API_KEY or self.config.ANTHROPIC_API_KEY == "":
                return (
                    "I'm sorry, but the system is not properly configured with an API key. Please contact your administrator.",
                    [],
                )
...
def enhanced_fastapi_endpoint():
    ...
    async def query_documents(request: QueryRequest):
        ...
        try:
            ...
            if (
                answer.startswith("I'm sorry")
                or answer.startswith("Authentication failed")
                or answer.startswith("The system is currently")
            ):
